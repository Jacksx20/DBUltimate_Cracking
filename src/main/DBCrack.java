 ...
 * <li>${DBEAVER_ECLIPSE_HOME}/plugins/com.dbeaver.lm.api.**.jar</li>
 * </ul>
 */
public class DBCrack {
    ...
    private static File privateKeyFile = new File(new File(System.getProperty("user.home"), ".jkiss-lm"),
            "private-key.txt");
    private static File publicKeyFile = new File(new File(System.getProperty("user.home"), ".jkiss-lm"),
            "public-key.txt");
    private static File licenseFile = new File(new File(System.getProperty("user.home"), ".jkiss-lm"), "license.txt");
    /**
     * 使用该文件，需要替换 jar 包中的 key 文件(请保留原始 jar 包), 并在 dbeaver.ini
     * 添加参数-Dlm.debug.mode=true
     * <p>
     * 如果修改 jar 包中的 plugin.xml 的 product 节点添加
     * <property name="lmServerURL" value="http://localhost:7879"/> 需要一个授权服务器
     * </p>
     */
    private static File jarPublicKey = new File(new File(System.getProperty("user.home"), ".jkiss-lm"),
            "dbeaver-ue-public.key");
     ...
     * @throws IOException
     */
    public static void generateKeyLicenseAndPatch(String jarFilePath, String filePathInJar)
            throws LMException, IOException {
